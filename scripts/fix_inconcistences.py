...
# ============================================================================
# 1. REGISTRY — Remover Entries Órfãos (5 componentes sem arquivo)
# ============================================================================
REGISTRY_FIXES = [
    # Formato: (texto_para_buscar, texto_para_substituir)
    ...
    if not file_path.exists():
        ...
        for search, replace in fixes:
            if search in content:
                ...
                if dry_run:
                    logger.debug(f"⚪ Não encontrado: {search[:50]}...")
...
def validate_registry() -> Tuple[int, int]:
    ...
    try:
        ...
        for comp_id, module_path in components.items():
            file_path = REPO_ROOT / f"{module_path.rsplit('.', 1)[0].replace('.', '/')}.py"
            if file_path.exists():
                ok += 1
            else:
                broken += 1
                logger.warning(f"❌ {comp_id}: {file_path.relative_to(REPO_ROOT)} NÃO EXISTE")
        
        return ok, broken
        
    except Exception as e:
        logger.error(f"❌ Erro ao validar registry: {e}")        return 0, 1
...
def main():
    ...
    if not args.dry_run and not args.apply and not args.validate:
        logger.error("❌ Use --dry-run, --apply ou --validate")
        sys.exit(1)
    
    logger.info("=" * 70)
    logger.info("🔧 JARVIS FIX INCONSISTENCIES — USO ÚNICO")
    logger.info("=" * 70)
    
    total_success = 0
    total_fail = 0
    
    # Aplica fixes por arquivo
    ...
    if args.validate or (args.apply and not args.dry_run):
        logger.info("\n" + "=" * 70)
        logger.info("📊 VALIDAÇÃO REGISTRY")
        logger.info("=" * 70)
        ok, broken = validate_registry()
        logger.info(f"✅ Componentes válidos: {ok}")
        logger.info(f"❌ Componentes quebrados: {broken}")
