...
try:
    ...
    from .state_machine import SelfHealingStateMachine, State, ErrorCategory, FailureReason
except ImportError:
...
"""
    ...
    def run_with_state_machine(self, pytest_report_path: Optional[str] = None) -> int:
        ...
        if state_machine.should_notify_human():
            # If it's an environment error, log and pause instead of mutating code
            if state_machine.error_category == ErrorCategory.ENVIRONMENT_ERROR:
                logger.warning(
                    ...
                    f"O sistema irá registrar no log e pausar em vez de tentar modificar o código."
                )
    ...
    def _attempt_repair(self, issue_body: str, issue_id: str) -> Dict[str, Any]:
        ...
        # 4.5. Validate syntax and run tests on temp file BEFORE touching real file
        file_suffix = Path(file_path).suffix or ".py"
        temp_path = self.repo_path / f"_temp_test_{issue_id}{file_suffix}"
        try:
            # Write fixed code to temp file for validation
            temp_path.write_text(fixed_code, encoding="utf-8")

            # Syntax check via compile() (Python files only)
            if file_suffix == ".py":
                try:
                    ...
                    return {
                        'success': False,
                        'file': file_path,
                        'description': 'Correção rejeitada pelos testes.',
                    }

            # Run pytest on tests/ to validate existing suite passes
            ...
            if pytest_result.returncode != 0:
                logger.error(
                    "❌ Pytest falhou — correção não aplicada.\nstdout: %s\nstderr: %s",
