...
11. **Self-Healing System**: Sistema de auto-reparo com roteamento inteligente de origem (GitHub Actions vs Jarvis User)

## Self-Healing Architecture

### Request Source Routing

O sistema implementa roteamento inteligente baseado na origem da solicitação, separando requisições automáticas do GitHub Actions de requisições manuais via Jarvis:

**GitHub Actions → Direct (sem identificador Jarvis)**
```
GitHub CI Failure → auto-heal.yml workflow → auto_fixer_logic.py
  - AUTO_FIXER_SOURCE=github_actions
  - Sem mission_id/session_id
  - Logs passados diretamente para AI
```

**Jarvis User → With Identifier (com rastreamento)**
```
User Request → AssistantService → GatewayLLMAdapter → GitHubAdapter
  - AUTO_FIXER_SOURCE=jarvis_user
  - Com mission_id e session_id
  - Rastreamento completo via ThoughtLog
```

### Components

#### 1. Auto-Fixer Script (`scripts/auto_fixer_logic.py`)
- Detecta origem via `AUTO_FIXER_SOURCE` environment variable
- Processa erros de CI ou requisições de usuário
- Usa AI (Groq/Gemini) para gerar correções
- Cria PRs automaticamente via GitHub CLI

#### 2. GitHub Adapter (`app/adapters/infrastructure/github_adapter.py`)
- `dispatch_auto_fix()`: Aceita parâmetro `source` ("github_actions" ou "jarvis_user")
- `create_issue()`: Adiciona informação de origem e identifiers opcionais no corpo da issue
- Apenas inclui `mission_id`/`session_id` quando `source="jarvis_user"`

#### 3. GitHub Workflows
- `.github/workflows/auto-heal.yml`: Trigger automático em CI failures
- `.github/workflows/jarvis_code_fixer.yml`: Trigger em issues com label `jarvis-auto-report`
- `.github/workflows/ci-failure-to-issue.yml`: Converte CI failures em GitHub issues

### Fluxo de Auto-Fix

**Cenário 1: CI Failure (GitHub Actions)**
```
1. Teste falha → ci-failure-to-issue.yml cria Issue
2. Issue criada → jarvis_code_fixer.yml triggered
3. Auto-fixer roda com AUTO_FIXER_SOURCE=github_actions
4. AI analisa logs e gera correção
5. PR criado automaticamente
6. Issue original fechada
```

**Cenário 2: User Request (Jarvis)**
```
1. Usuário reporta erro via Jarvis
2. AssistantService processa com mission_id
3. GatewayLLMAdapter detecta erro crítico
4. GitHubAdapter.dispatch_auto_fix() chamado com source=jarvis_user
5. ThoughtLog registra tentativa com mission_id/session_id
6. Auto-fixer roda com identifiers para rastreamento
```

### Migration Notes

- `GitHubWorker.auto_heal_ci_failure()` está **deprecated**
- Use `scripts/auto_fixer_logic.py` para novas implementações
- Use `GitHubAdapter.dispatch_auto_fix()` para integração programática
