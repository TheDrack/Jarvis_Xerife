...
CORRIGIDO: Proteção contra KeyError na chave 'results' e merge de contexto.
"""
import os
import yaml
import logging
...
def run_pipeline(pipeline_name: str, global_strict: bool = False) -> Dict[str, Any]:
    ...
    for step_name, meta in components.items():
        ...
        if not instance or getattr(instance, "__is_cloud_mock__", False):
            ...
            if is_strict: raise RuntimeError(f"❌ [STRICT] {msg}")
            continue
        ...
        try:
            ...
            # --- CORREÇÃO DO BUG: Garantir que 'results' exista após a execução ---
