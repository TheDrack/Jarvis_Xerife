...
def run_pipeline(pipeline_name: str, global_strict: bool = False) -> Dict[str, Any]:
    ...
    if not components:
        logger.warning(f"⚠️ O pipeline '{pipeline_name}' não definiu nenhum componente.")
        return context
    ...
    for step_name, meta in components.items():
        ...
        if getattr(instance, "__is_cloud_mock__", False):
            ...
            if is_strict:
                raise RuntimeError(f"❌ [STRICT] {msg} Interrompendo execução real.")
        ...
        try:
            ...
            if hasattr(instance, "can_execute") and not instance.can_execute(context):
                logger.info(f"⏭️ Passo '{step_name}' pulado (can_execute=False).")
                continue
            ...
            if isinstance(updated_context, dict):
                ...
                logger.warning(f"⚠️ {step_name} retornou {type(updated_context)} em vez de dict. Contexto mantido.")
            ...
            logger.warning(f"⚠️ {step_name} falhou. Prosseguindo conforme configuração.")
