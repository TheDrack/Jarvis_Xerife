...
class TelegramAdapter(NexusComponent):
    ...
    def configure(self, config: dict):
        """Configuração via Pipeline YAML"""
        self.config_data = config

    def send_message(self, text: str, chat_id: Any = None) -> Any:
        target = chat_id or self.chat_id
        if not target or not self.token: return None
        try:
            url = f"{self.base_url}/sendMessage"
            return requests.post(url, json={"chat_id": target, "text": text}, timeout=10)
    ...
    def _action_upload_backup(self, context: dict) -> dict:
        logger.info("📤 [TELEGRAM] Ação de backup detectada no pipeline.")
    ...
    def _poll_loop(self, callback: Callable, interval: float):
        while self._polling:
            try:
                ...
                logger.error(f"⚠️ [TELEGRAM] Erro no loop: {e}")
                time.sleep(5) # Espera mais tempo em caso de erro de rede

    def get_updates(self, offset: int = 0, timeout: int = 10) -> list:
        if not self.token: return []
        try:
            # Usando requests direto para evitar qualquer overhead do HttpClient customizado no polling
    ...
    def handle_update(self, update: Dict[str, Any], callback: Optional[Callable] = None) -> None:
        ...
        if text and callback:
            ...
            if response:
                self.send_message(str(response), chat_id=chat_id)
