...
CORREÇÃO: Implementação de Streaming para economia de memória e suporte a Async.
"""
import ast
import logging
...
# Mock/Import do Nexus conforme estrutura do projeto
try:
...
class Consolidator(NexusComponent):
                ...
                if isinstance(node, ast.ClassDef):
                    ...
                    for item in node.body:
                        if isinstance(item, (ast.FunctionDef, ast.AsyncFunctionDef)):
                            ...
                            skeleton.append(f"    {prefix}def {item.name}({', '.join(args)}...): ...")
                
                # Trata Funções Top-level (Síncronas e Assíncronas)
                elif isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                    prefix = "async " if isinstance(node, ast.AsyncFunctionDef) else ""
                    args = [a.arg for a in node.args.args[:3]]
                    skeleton.append(f"{prefix}def {node.name}({', '.join(args)}...): ...")
            
            return "\n".join(skeleton) if skeleton else "# (Nenhuma classe ou função detectada)"
        except SyntaxError:
            return "# Erro: Falha de sintaxe no arquivo original. Skeleton parcial indisponível."
    ...
    def execute(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Lista temporária para o Mapa de Assinaturas (Seção 1)
        # Manter o skeleton em memória é aceitável pois é leve
        skeleton_map = []
        ...
        try:
            with open(output_path, "w", encoding="utf-8") as out:
                ...
                # Processamento Fase 1: Coleta de Skeletons
                for file_path in all_files:
                    rel_path = str(file_path.relative_to(self.root_path))
                    layer = self._get_layer_info(rel_path)
                    size = file_path.stat().st_size
                    skel = self._get_skeleton(file_path)
                    skeleton_map.append(f"[{layer}] {rel_path} ({size} bytes):\n{skel}\n" + "-"*30)
                ...
                # Processamento Fase 2: Escrita em Stream (Dense Content)
                for file_path in all_files:
                    rel_path = str(file_path.relative_to(self.root_path))
                    layer = self._get_layer_info(rel_path)
                    ...
                    try:
                        # Lê e escreve imediatamente para não saturar a RAM
                        content = file_path.read_text(encoding="utf-8", errors="replace")
                        out.write(content)
                        out.write("\n")
                    except Exception as e:
