...
def create_api_server(assistant_service: AssistantService, extension_manager: ExtensionManager = None) -> FastAPI:
    ...
    async def telegram_webhook(data: dict, background_tasks: BackgroundTasks):
        ...
        def telegram_webhook_callback(text: str, chat_id: str) -> str:
            """Wrapper to call process_command with telegram channel context."""
            ...
            if isinstance(response, dict):
                return str(response.get("result") or response.get("error", ""))
            return str(response)

        background_tasks.add_task(telegram.handle_update, data, telegram_webhook_callback)
        return {"ok": True}
