...
from .overwatch_inactivity import InactivityMonitor, suggest_pending_task
...
class OverwatchDaemon(ResourceMonitor, PerimeterMonitor, NexusComponent):
    ...
    # ------------------------------------------------------------------
    def start(self) -> None:
        """Inicia o daemon em thread background."""
        if self._running:
            return
    ...
    def notify_activity(self) -> None:
        """Notifica atividade do usuário (reseta timer de inatividade)."""
        self._inactivity.reset_timer()
    ...
    def _check_perimeter(self) -> None:
        """Verifica perímetro tático a cada N ticks."""
    ...
    def _store_trace(self, mac: str, soldier_id: str, device: Any) -> None:
        """Armazena rastro forense na memória vetorial."""
        try:
            vector_mem = nexus.resolve("vector_memory_adapter")
            if vector_mem:
                narrative = f"Intrusão: MAC={mac}, Soldado={soldier_id}"
                vector_mem.store_event(narrative, metadata={"mac": mac})
        except Exception:
            pass
    
    def _compile_modules(self) -> None:
        """Compila módulos JRVS."""
        try:
