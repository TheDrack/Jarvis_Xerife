...
def save_env_file(
    ...
    # Add user-specific settings at the beginning (after comments)
    user_settings = [
        "\n# User Settings\n",
        f"USER_ID={user_id}\n",
    ...
    # Find the position to insert (after initial comments)
    insert_pos = 0
    ...
    env_content = env_content[:insert_pos] + user_settings + env_content[insert_pos:]
    
    # Ensure API key and database URL are set
    has_api_key = any('GEMINI_API_KEY=' in line for line in env_content)
    has_db_url = any('DATABASE_URL=' in line for line in env_content)
